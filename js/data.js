...
SWM.loadData = async function(){
  ...
  try {
    ...
    let res = await fetch('data/content.json', {cache:'no-store'});
    
    // إذا لم يعمل (لـ GitHub Pages)
    if (!res.ok) {
      res = await fetch('/Study-With-Me-/data/content.json', {cache:'no-store'});
    }
    ...
    if (box) { 
      box.hidden = false; 
      box.textContent = `تعذر تحميل المحتوى: ${err.message}`; 
    }
    return null;
  }
};
...
SWM.typeName = function(type){ return ({folder:'مجلد',pdf:'PDF',exam:'اختبار',quiz:'اختبار',form:'Google Forms',document:'Google Docs',telegram:'Telegram',external:'مصدر خارجي'}[type]||type); };
